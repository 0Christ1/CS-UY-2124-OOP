...
public:
    ...
    int getWeaponStrength() const {return weapon.getWeaponStrength();}
    
    // setter for weapon_strength
    void setWeaponStrength(int weaponStrength) {
        weapon.setWeaponStrength(weaponStrength);
    }
    ...
    //         the loser would be dead and winner's weapon strength would be changed
        ...
        int new_strength =
            warrior2.getWeaponStrength()
                - weapon.getWeaponStrength();
        weapon.setWeaponStrength(0);
        warrior2.setWeaponStrength(new_strength);
    }
    
    // Case 4: if they were both dead, they wouldn't fight as well.
    else if (weapon.getWeaponStrength() == 0
             && warrior2.getWeaponStrength() == 0) {
