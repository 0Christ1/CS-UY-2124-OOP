...
class Directory {
    ...
    public:
        Entry(const string &name, unsigned room, unsigned phone,
              Position &position) : name(name), room(room), phone(phone),
                                    pos(&position) {
        }
...
public:
    ...
    // ???
    // ...
    Directory(const string &in_company) : company(in_company) {
        size = 0;
        capacity = 0;
    ...
    Directory(const Directory &rhs) :
            company(rhs.company), size(rhs.size), capacity(rhs.capacity) {
        entries = new Entry *[capacity];
    ...
    void add(const string &name, unsigned room, unsigned ph, Position &pos) {
        if (size == capacity) {
            ...
            if (capacity == 0) {
                ...
                capacity *= 2;
                Entry **nextEntry = new Entry *[capacity];
                for (size_t i = 0; i < size; ++i) {
                    nextEntry[i] = entries[i];
                }
                delete[] entries;
                entries = nextEntry;
            }
